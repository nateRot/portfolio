...
class FirstLastSampler(Sampler):
    ...
    def __iter__(self) -> Iterator[int]:
        ...
        self.count = 0
        self.reverse = False
        while self.count < len(self.data_source):
            if not self.reverse:
                yield self.num
                self.num += 1
                self.count += 1
            elif self.reverse:
                yield len(self.data_source) - self.num
                self.count += 1
...
def create_train_validation_loaders(
    ...
    val_split_index = int(np.floor(validation_ratio * dataset_size))
    ...
    dl_train = torch.utils.data.DataLoader(dataset, shuffle=False, batch_size=batch_size, num_workers=num_workers, sampler = train_sampler)
    dl_valid = torch.utils.data.DataLoader(dataset, shuffle=False, batch_size=batch_size, num_workers=num_workers, sampler = val_sampler)
