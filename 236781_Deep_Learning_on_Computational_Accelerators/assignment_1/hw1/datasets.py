...
def random_labelled_image(
    ...
    # ====== YOUR CODE: ======
    image = torch.randint(high, shape)
    label = torch.randint(num_classes - 1, (1,1))
    label = label.item()
    # ========================
...
class RandomImageDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> Tuple[Tensor, int]:
        ...
        with torch_temporary_seed(index):
            image_label = random_labelled_image(self.image_dim, self.num_classes)
        return image_label            
        # ========================
