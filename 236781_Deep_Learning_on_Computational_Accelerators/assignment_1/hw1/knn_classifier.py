...
import hw1.dataloaders as dataloaders
import hw1.datasets as datasets
...
class KNNClassifier(object):
    ...
    def predict(self, x_test: Tensor):
        ...
        # i added
        k_closest_values, k_closest_indicies = torch.topk(dist_matrix, self.k, dim=0, largest=False)
        ...
        for i in range(n_test):
            ...
            # ====== YOUR CODE: ======
            k_labels = self.y_train[k_closest_indicies[: , i]]
...
def l2_dist(x1: Tensor, x2: Tensor):
    ...
    dists = None
    # ====== YOUR CODE: ======
    sum_x1 = torch.sum(x1**2,dim=1)
    sum_x2 = torch.sum(x2**2,dim=1)

    sum_x1 = sum_x1.unsqueeze(1) 
    mult = torch.mm(x1,x2.t())
    mult = 2*mult
...
def accuracy(y: Tensor, y_pred: Tensor):
    ...
    accuracy = total_correct.item() / y.numel()
    # raise NotImplementedError()
    # ========================
