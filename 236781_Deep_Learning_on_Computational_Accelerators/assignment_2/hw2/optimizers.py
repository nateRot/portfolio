...
class MomentumSGD(Optimizer):
    ...
    def step(self):
        for p, dp in self.params:
            ...
            # ====== YOUR CODE: ======
            self.velocity[self.curr_vel] *= self.momentum
            dp += p * self.reg
            self.velocity[self.curr_vel] -= self.learn_rate*dp
            p += self.velocity[self.curr_vel]

            if self.curr_vel + 1 == len(self.velocity):
                self.curr_vel = 0
            else:
                self.curr_vel += 1
            # raise NotImplementedError()
...
class RMSProp(Optimizer):
    ...
    def step(self):
        for p, dp in self.params:
            ...
            # parameters tensor.
            # ====== YOUR CODE: ======
            # self.r[self.curr_r] *= self.decay
            # dp +=  p * self.reg
            # self.r[self.curr_r] += (1 - self.decay)*(dp**2)
            # p = p - (self.learn_rate/(torch.sqrt(self.r[self.curr_r] + self.eps)))*dp

            # if self.curr_r + 1 == len(self.r):
            #     self.curr_r = 0
            # else:
            #     self.curr_r += 1
            curr_idx = self.curr_r    # Get current index to choose rate

            # Update parameter, gradients and velocity
            dp += self.reg * p
            self.r[curr_idx] *= self.decay
            self.r[curr_idx] += (1 - self.decay)*(dp**2)
            p -= (self.learn_rate/(torch.sqrt(self.r[curr_idx] + self.eps)))*dp

            # Update index of next velocity element (or zero it if we reached the end)
            if self.curr_r == len(self.r) - 1:
                self.curr_r = 0
            else:
                self.curr_r += 1
            # raise NotImplementedError()
