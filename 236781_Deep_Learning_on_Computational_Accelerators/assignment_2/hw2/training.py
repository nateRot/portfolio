    ...
    def fit(
        ...
        **kw,
        ...
        for epoch in range(num_epochs):
            ...
            # Early stopping
            if not(early_stopping is None) and actual_num_epochs >= early_stopping:

                last_losses = test_loss[-early_stopping:]

                if all(i <= j for i, j in zip(last_losses, last_losses[1:])):
                    break

            # Checkpoint
            if actual_num_epochs > 1 and not(checkpoints is None):
...
class BlocksTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        # ====== YOUR CODE: ======
        loss = self.loss_fn(self.model(X), y=y).item()
    ...
    def test_batch(self, batch) -> BatchResult:
        ...
        loss = self.loss_fn(class_scores, y=y).item()
...
class TorchTrainer(Trainer):
    ...
    def test_batch(self, batch) -> BatchResult:
        ...
        with torch.no_grad():
            ...
            # ====== YOUR CODE: ======
            score = self.model(X)
            loss = self.loss_fn(score, y).item()
