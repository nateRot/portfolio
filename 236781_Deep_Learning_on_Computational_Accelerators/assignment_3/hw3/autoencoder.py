...
class VAE(nn.Module):
    ...
    def encode(self, x):
        ...
        z = mu + (log_sigma2.mul(0.5).exp_())*torch.randn_like(mu)
        # raise NotImplementedError()
        # ========================
    ...
    def decode(self, z):
        ...
        reconstructed_in = self.features_decoder(reconstructed_in.view([-1, self.features_shape[0], self.features_shape[1], self.features_shape[2]]))
    ...
    def sample(self, n):
        ...
        with torch.no_grad():
            ...
            for i in range(n):
                ...
                randomized_z = torch.randn(self.z_dim, device=device)
                reconstructions = self.decode(randomized_z)
                # reconstructed samples added
                samples.append(reconstructions[0, :, :, :])
            # raise NotImplementedError()
            # ========================
...
def vae_loss(x, xr, z_mu, z_log_sigma2, x_sigma2):
    ...
    # ====== YOUR CODE: ======
    data_loss = torch.mean((1/(x_sigma2*x[0, :, :, :].numel()))*torch.norm(x.view(x.shape[0], -1) - xr.view(xr.shape[0], -1), dim=1)**2)
    kldiv_loss = torch.mean(torch.sum(torch.exp(z_log_sigma2), 1) + torch.norm(z_mu, dim=1)**2 - z_mu.size(1) - \
                          torch.log(torch.prod(torch.exp(z_log_sigma2), dim=1)))
