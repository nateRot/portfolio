...
def onehot_to_chars(embedded_text: Tensor, idx_to_char: dict) -> str:
    ...
    # ====== YOUR CODE: ======
    result = ''
    for one_hot_vec in embedded_text:
        idx = (one_hot_vec == 1).nonzero().item()
...
def chars_to_labelled_samples(text: str, char_to_idx: dict, seq_len: int, device="cpu"):
    ...
    # leave out remainder, if it doesn't fill a sequence
    remain = (len(text) - 1)%seq_len

    embedding = chars_to_onehot(text, char_to_idx)
    samples = embedding[:-(remain+1), :]
    # splitting in samples
    samples = samples.view(N, seq_len, embedding.size(1))
    # for i in text[1:-remain]:
    #     labels = torch.tensor(char_to_idx[i], dtype=torch.int8)
    labels = torch.tensor([char_to_idx[i] for i in text[1:-remain]], dtype=torch.int8)
...
def hot_softmax(y, dim=0, temperature=1.0):
    ...
    # ====== YOUR CODE: ======
    result = nn.functional.softmax(y*(1/temperature), dim=dim)
    # raise NotImplementedError()
    # ========================
    return result
...
def generate_from_model(model, start_sequence, n_chars, char_maps, T):
    ...
    with torch.no_grad():
        ...
        # initializing model feed
        _chars_onehot = chars_to_onehot(start_sequence, char_to_idx).to(device)
        _chars_onehot = _chars_onehot.view(-1, *_chars_onehot.shape).float()
        q = _chars_onehot * 2
        ...
        for i in range(n_chars - len(start_sequence)):
            ...
            y, h_s = model(_chars_onehot, h_s)   

            y_softmax = (y[:, -1, :]).flatten() 

            y_softmax = hot_softmax(y_softmax, temperature=T)   
            pred_idx = y_softmax.multinomial(num_samples=1)   

            generated_char = idx_to_char[pred_idx.item()]
            out_text += generated_char   

            _chars_onehot = chars_to_onehot(generated_char, char_to_idx).to(device)
            _chars_onehot = _chars_onehot.view(-1, *_chars_onehot.shape).float()
    # raise NotImplementedError()
    # ========================
...
class SequenceBatchSampler(torch.utils.data.Sampler):
    ...
    def __iter__(self) -> Iterator[int]:
        ...
        # ====== YOUR CODE: ======
        number_of_batches = math.ceil(len(self.dataset) // self.batch_size)
        idx = []
        for i in range(number_of_batches):
            idx += [j*number_of_batches + i for j in range(self.batch_size)]
...
class MultilayerGRU(nn.Module):
    ...
    def __init__(self, in_dim, h_dim, out_dim, n_layers, dropout=0):
        ...
        for i in range(n_layers):
            ...
            self.layer_params[-1].append(nn.Linear(in_dim, h_dim, bias=False))
            string = 'W_xz_' + str(i)
            self.add_module(string, self.layer_params[-1][-1])
            self.layer_params[-1].append(nn.Linear(h_dim, h_dim))
            string = 'W_hz_' + str(i)
            self.add_module(string, self.layer_params[-1][-1])
            self.layer_params[-1].append(nn.Linear(in_dim, h_dim, bias=False))
            string = 'W_xr_' + str(i)
            ...
            self.layer_params[-1].append(nn.Dropout(dropout))
            string = 'dropout_' + str(i)
            self.add_module(string, self.layer_params[-1][-1])

            in_dim = h_dim
        # Output
        self.fc_y = nn.Linear(h_dim, out_dim)
        string = 'W_hy'
        self.add_module(string, self.fc_y)
        # raise NotImplementedError()
        # ========================
    ...
    def forward(self, input: Tensor, hidden_state: Tensor = None):
        ...
        for t in range(seq_len):
            ...
            for i, hidden_layer in enumerate(self.layer_params):
                
                # z = \sig(x, h)
                z = nn.functional.sigmoid(hidden_layer[1](layer_states[i]) + hidden_layer[0](x))
                # r = \sig(x, h)
                r = nn.functional.sigmoid(hidden_layer[3](layer_states[i]) + hidden_layer[2](x))
                # g = \tanh(x, r, h)
                g = nn.functional.tanh(hidden_layer[5](r*layer_states[i]) + hidden_layer[4](x))
                # h = f(z, h)
