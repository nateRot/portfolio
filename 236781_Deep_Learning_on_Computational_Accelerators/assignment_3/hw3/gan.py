...
def discriminator_loss_fn(y_data, y_generated, data_label=0, label_noise=0.0):
    ...
    # ====== YOUR CODE: ======
    noisyData = torch.rand(y_generated.shape, device=y_data.device)*label_noise - (label_noise/2) + data_label
    noisyGen = torch.rand(y_generated.shape, device=y_data.device)*label_noise - (label_noise/2)

    LogitLoss = nn.BCEWithLogitsLoss()

    loss_data = LogitLoss(y_data, noisyData)
    loss_generated = LogitLoss(y_generated, noisyGen)
    # raise NotImplementedError()
    # ========================
...
def train_batch(
    ...
    dsc_out = dsc_model(x_data)

    gen_sample = gen_model.sample(x_data.shape[0], with_grad=True)
    dsc_out_gen = dsc_model(gen_sample.detach())

    dsc_loss = dsc_loss_fn(dsc_out, dsc_out_gen)
    ...
    dsc_out_gen = dsc_model(gen_sample)

    gen_loss = gen_loss_fn(dsc_out_gen)
