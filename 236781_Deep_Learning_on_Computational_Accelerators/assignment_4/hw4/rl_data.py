...
class Episode(object):
    ...
    def calc_qvals(self, gamma: float) -> List[float]:
        ...
        for exp in reversed(self.experiences):
            sum_q = sum_q * gamma + exp.reward
            qvals.insert(0, sum_q)
        # ========================
        return qvals
...
class TrainBatch(object):
    ...
    def from_episodes(cls, episodes: Iterable[Episode], gamma=0.999):
        ...
        # ====== YOUR CODE: ======
        tens_state = torch.tensor((), dtype=torch.float)
        tens_act = torch.tensor((), dtype=torch.long)
        q_vals = torch.tensor((), dtype=torch.float)
        tens_total = torch.tensor((), dtype=torch.float)
        
        init_calc = True
        for curr_episode in episodes:
            episode_qval = curr_episode.calc_qvals(gamma)
            if init_calc:
                q_vals = torch.FloatTensor(episode_qval)
                tens_total = torch.FloatTensor([curr_episode.total_reward]).unsqueeze(0)
            else:
                episode_qval_tensor = torch.FloatTensor(episode_qval)
                q_vals = torch.cat((q_vals, episode_qval_tensor))
                total = torch.FloatTensor([curr_episode.total_reward]).unsqueeze(0) 
                tens_total = torch.cat((tens_total, total), dim=0)

            for exp in curr_episode.experiences:
                if init_calc:
                    tens_state = torch.FloatTensor(exp.state).unsqueeze(0)
                    tens_act = torch.LongTensor([exp.action]).unsqueeze(0)
                    init_calc = False
                else:
                    tens_state = torch.cat((tens_state, exp.state.unsqueeze(0)), dim=0)
                    exp_action_tensor = torch.LongTensor([exp.action]).unsqueeze(0)
                    tens_act = torch.cat((tens_act, exp_action_tensor), dim=0)
        train_batch = cls(tens_state, tens_act, q_vals, tens_total)
        # ========================
        return train_batch
    ...
    def episode_batch_generator(self) -> Iterator[Tuple[Episode]]:
        ...
        while True:
            ...
            while True:
                ...
                if step_exp.is_done:
                    break
            single_eps = Episode(episode_reward, episode_experiences)
            curr_batch.append(single_eps)
            episode_reward = 0.0
            episode_experiences = []
            # ========================
