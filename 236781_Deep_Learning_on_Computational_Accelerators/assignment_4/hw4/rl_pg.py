...
class PolicyNet(nn.Module):
    def __init__(self, in_features: int, out_actions: int, **kw):
        ...
        layer = []
        layer.append(torch.nn.Linear(self.in_features, 512))
        layer.append(torch.nn.ReLU())
        layer.append(torch.nn.Linear(512, 256))
        layer.append(torch.nn.ReLU())
        layer.append(torch.nn.Linear(256, self.out_actions))
        self.policy_nn = torch.nn.Sequential(*layer)
        # ========================

    def forward(self, x):
        # TODO: Implement a simple neural net to approximate the policy.
        # ====== YOUR CODE: ======
        action_scores = self.policy_nn(x)
        # ========================
    ...
    def build_for_env(env: gym.Env, device="cpu", **kw):
        ...
        net = PolicyNet(input_d.shape[0], 4).to(device)
        # ========================
...
class PolicyAgent(object):
    ...
    def current_action_distribution(self) -> torch.Tensor:
        ...
        # ====== YOUR CODE: ======
        with torch.no_grad():
            actions_proba = torch.nn.functional.softmax(self.p_net(self.curr_state),dim=0)
        # ========================
    ...
    def step(self) -> Experience:
        ...
        take_action = torch.multinomial(self.current_action_distribution(), 1).item()
        
        next_state, reward, is_done, _ = self.env.step(take_action)
        next_state = torch.tensor(next_state, device=self.device)
        
        self.curr_episode_reward += reward
        experience = Experience(self.curr_state, take_action, reward, is_done)
...
class VanillaPolicyGradientLoss(nn.Module):
    ...
    def _policy_loss(self, batch, action_scores, policy_weight):
        ...
        # ====== YOUR CODE: ======
        log_softmax = torch.nn.functional.log_softmax(action_scores, dim=-1)
        log_softmax_gather = torch.gather(log_softmax, dim=1, index=batch.actions)
        log_prob = log_softmax_gather.reshape(log_softmax_gather.shape[0])
        loss_p = torch.dot(policy_weight,log_prob)
        loss_p = -1 * loss_p / len(batch)
        # ========================
        return loss_p
...
class BaselinePolicyGradientLoss(VanillaPolicyGradientLoss):
    def forward(self, batch: TrainBatch, action_scores: torch.Tensor, **kw):
        ...
        log_softmax = torch.nn.functional.log_softmax(action_scores, dim=-1)
        log_softmax_gather = torch.gather(log_softmax, dim=1, index=batch.actions)
        log_prob = log_softmax_gather.view(log_softmax_gather.shape[0])

        policy_weight, baseline = self._policy_weight(batch)
        policy_weight = policy_weight - baseline
        loss_p = torch.dot(policy_weight,log_prob)
        loss_p = -1 * loss_p / len(batch)
        # ========================
...
class ActionEntropyLoss(nn.Module):
    ...
    def forward(self, batch: TrainBatch, action_scores, **kw):
        ...
        # ====== YOUR CODE: ======
        soft_max = torch.nn.functional.softmax(action_scores, dim=1)*torch.nn.functional.log_softmax(action_scores, dim=1)
        soft_max = soft_max.sum(dim=1)
        soft_max = torch.mean(soft_max)/self.max_entropy
        loss_e = soft_max
        # ========================
...
class PolicyTrainer(object):
    ...
    def train_batch(self, batch: TrainBatch):
        ...
        for loss_func in self.loss_functions:
            loss, current_loss_dict = loss_func.forward(batch, action_scores)
            total_loss += loss
            losses_dict.update(current_loss_dict)
